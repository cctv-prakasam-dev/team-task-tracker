// General Response Messages
export const DEF_SERVICE_RUNNING = "Service is up and running";
export const DEF_SUCCESS_RESP = "Success";
export const DEF_ERROR_RESP = "Internal server error";

// HTTP Error Messages
export const DEF_422 = "Validation failed";
export const DEF_404 = "Data not found";
export const DEF_409 = "Data conflict encountered";
export const DEF_401 = "Unauthorized request";
export const DEF_403 = "Forbidden request";
export const DEF_400 = "Bad request";

export const NAME_422 = "VALIDATION_ERROR";
export const NAME_404 = "NOT_FOUND";
export const NAME_409 = "CONFLICT";
export const NAME_401 = "UNAUTHORIZED";
export const NAME_403 = "FORBIDDEN";
export const NAME_400 = "BAD_REQUEST";

export const FORBIDDEN_ACCESS = "Forbidden access";
export const DESCRIPTION_MISSING = "Description is missing.";
export const NEXT_HEARING_DATE_MISSING = "Next hearing date is missing.";
export const CASES_NOT_FOUND_FOR_PRIORITY_UPDATE = "No cases found for priority update";
export const TOKEN_INVALID = "Invalid token";
export const TOKEN_EXPIRED = "Token expired";
export const TOKEN_MISSING = "Token missing";
export const TOKEN_SIG_MISMATCH = "Token signature mismatch";
export const USER_INACTIVE = "User is inactive";