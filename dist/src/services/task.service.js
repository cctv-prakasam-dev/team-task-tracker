...
async function getOrgProjectIds(orgId) {
    const orgProjects = await getMultipleRecordsByAColumnValue(projects, "org_id", orgId, "eq");
    return orgProjects.map(p => p.id);
}
async function createTask(input, orgId, userId) {
    const projectIds = await getOrgProjectIds(orgId);
    if (!projectIds.includes(input.project_id))
        throw new NotFoundException("Project not found");
    if (input.assignee_id) {
        const assignee = await getSingleRecordByMultipleColumnValues(users, ["id", "org_id"], [input.assignee_id, orgId], ["eq", "eq"]);
        if (!assignee)
            throw new NotFoundException("Assignee not found in organization");
    }
    const task = await saveSingleRecord(tasks, { ...input, created_by: userId });
    if (input.assignee_id) {
        await cacheService.invalidateTasksCache(input.assignee_id);
    }
    return task;
}
async function listTasks(query, orgId, requestingUserId, requestingUserRole) {
...
async function getTaskById(id, orgId, requestingUserId, requestingUserRole) {
    const projectIds = await getOrgProjectIds(orgId);
    if (projectIds.length === 0)
...
async function updateTask(id, orgId, input, requestingUserId, requestingUserRole) {
    ...
    const updated = await updateRecordById(tasks, id, input);
    if (oldAssigneeId)
        await cacheService.invalidateTasksCache(oldAssigneeId);
    if (input.assignee_id && input.assignee_id !== oldAssigneeId) {
        await cacheService.invalidateTasksCache(input.assignee_id);
    }
    return updated;
}
async function updateTaskStatus(id, orgId, newStatus, requestingUserId, requestingUserRole) {
    const task = await getTaskById(id, orgId, requestingUserId, requestingUserRole);
    const canChange = requestingUserRole === "ADMIN"
        || requestingUserRole === "MANAGER"
        || task.assignee_id === requestingUserId;
    if (!canChange)
        throw new ForbiddenException("Only the assignee or a manager can update task status");
    const allowed = VALID_TRANSITIONS[task.status];
    if (!allowed.includes(newStatus)) {
        throw new BadRequestException(`Invalid status transition from ${task.status} to ${newStatus}. Allowed: ${allowed.join(", ") || "none"}`);
    }
    const updated = await updateRecordById(tasks, id, { status: newStatus });
    if (task.assignee_id)
        await cacheService.invalidateTasksCache(task.assignee_id);
    return updated;
}
async function deleteTask(id, orgId, requestingUserId, requestingUserRole) {
    const task = await getTaskById(id, orgId, requestingUserId, requestingUserRole);
    await deleteRecordById(tasks, id);
    if (task.assignee_id)
        await cacheService.invalidateTasksCache(task.assignee_id);
    return { id };
}
