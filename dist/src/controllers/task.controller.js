...
export async function getTaskById(c) {
    const user = c.get("user");
...
export async function updateTaskStatus(c) {
    const user = c.get("user");
    const id = Number(c.req.param("id"));
    const body = await c.req.json();
    const { status } = updateTaskStatusSchema.parse(body);
    const task = await taskService.updateTaskStatus(id, user.org_id, status, user.id, user.role);
    return sendSuccessResp(c, 200, "Task status updated", { task });
}
export async function deleteTask(c) {
    const user = c.get("user");
    const id = Number(c.req.param("id"));
